...
def test_execution_environment_not_set_and_docker_context(monkeypatch):
    ...
    import importlib
    import infra.agent_nat as agent_infra
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_nat] Execution Environment"
    assert kwargs["programming_language"] == "python"
    assert kwargs["name"] == "[unittest] [agent_nat] Execution Environment"
    assert kwargs["description"] == "Execution Environment for [unittest] [agent_nat]"  # fmt: skip
...
def test_custom_model_created(monkeypatch):
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_nat] Custom Model"
    assert kwargs["name"] == "[unittest] [agent_nat] Custom Model"
    assert kwargs["base_environment_id"] == agent_infra.agent_nat_execution_environment.id  # fmt: skip
    assert (
        kwargs["base_environment_version_id"]
        == agent_infra.agent_nat_execution_environment.version_id
    )
...
def test_agentic_playground_and_blueprint_created(monkeypatch):
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_nat] Agentic Playground"
    assert kwargs["name"] == "[unittest] [agent_nat] Agentic Playground"
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_nat] LLM Blueprint"
    assert kwargs["name"] == "[unittest] [agent_nat] LLM Blueprint"
    ...
    assert kwargs[
        ...
        custom_model_id=agent_infra.agent_nat_custom_model.id
    )
    ...
    assert "Agent Playground URL " + agent_infra.agent_nat_asset_name in export_names  # fmt: skip
    ...
    agent_infra.pulumi.Output.format.assert_any_call(
        ...
        "mock-use-case-id",
        agent_infra.agent_nat_playground.id,
    )
...
def test_agent_deployment_created_when_env(monkeypatch):
    ...
    agent_infra.pulumi.export.assert_any_call(
        "Agent Deployment Chat Endpoint " + agent_infra.agent_nat_asset_name,
...
class TestSynchronizePyprojectDependencies:
    def test_synchronize_pyproject_dependencies_basic(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(agent_infra, "agent_nat_application_path", tmp_path)
