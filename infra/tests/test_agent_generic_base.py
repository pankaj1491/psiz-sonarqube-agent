...
def test_execution_environment_not_set_and_docker_context(monkeypatch):
    ...
    assert (
        ...
        == "[unittest] [agent_generic_base] Execution Environment"
    )
    assert kwargs["programming_language"] == "python"
    assert kwargs["name"] == "[unittest] [agent_generic_base] Execution Environment"
...
def test_custom_model_created(monkeypatch):
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_generic_base] Custom Model"
    assert kwargs["name"] == "[unittest] [agent_generic_base] Custom Model"
...
def test_agentic_playground_and_blueprint_created(monkeypatch):
    ...
    assert (
        kwargs["resource_name"] == "[unittest] [agent_generic_base] Agentic Playground"
    )
    assert kwargs["name"] == "[unittest] [agent_generic_base] Agentic Playground"
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_generic_base] LLM Blueprint"
    assert kwargs["name"] == "[unittest] [agent_generic_base] LLM Blueprint"
    ...
    assert "Agent Playground URL " + agent_infra.agent_generic_base_asset_name in export_names  # fmt: skip
...
def test_agent_deployment_created_when_env(monkeypatch):
    ...
    agent_infra.pulumi.export.assert_any_call(
        "Agent Deployment Chat Endpoint " + agent_infra.agent_generic_base_asset_name,
...
class TestSynchronizePyprojectDependencies:
    def test_synchronize_pyproject_dependencies_basic(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            agent_infra, "agent_generic_base_application_path", tmp_path
        )
