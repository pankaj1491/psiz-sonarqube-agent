...
def test_execution_environment_not_set_and_docker_context(monkeypatch):
    ...
    assert kwargs["name"] == "[unittest] [agent_llamaindex] Execution Environment"
    assert kwargs["description"] == "Execution Environment for [unittest] [agent_llamaindex]"  # fmt: skip
...
def test_custom_model_created(monkeypatch):
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_llamaindex] Custom Model"
    assert kwargs["name"] == "[unittest] [agent_llamaindex] Custom Model"
    assert kwargs["base_environment_id"] == agent_infra.agent_llamaindex_execution_environment.id  # fmt: skip
    assert (
        kwargs["base_environment_version_id"]
        == agent_infra.agent_llamaindex_execution_environment.version_id
    )
...
def test_agentic_playground_and_blueprint_created(monkeypatch):
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_llamaindex] Agentic Playground"
    assert kwargs["name"] == "[unittest] [agent_llamaindex] Agentic Playground"
    ...
    assert kwargs["resource_name"] == "[unittest] [agent_llamaindex] LLM Blueprint"
    assert kwargs["name"] == "[unittest] [agent_llamaindex] LLM Blueprint"
...
def test_agent_deployment_created_when_env(monkeypatch):
    ...
    agent_infra.pulumi.export.assert_any_call(
        "Agent Deployment Chat Endpoint " + agent_infra.agent_llamaindex_asset_name,
